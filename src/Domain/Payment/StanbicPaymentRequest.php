...
/**
 * Stanbic internal payment request.
 *
 * @psalm-immutable
*/
final class StanbicPaymentRequest extends PaymentRequest
{
    ...
    */
    public static function fromArray(array $data): self
    {
        ...
        if (empty($originatorAccount->identification->toArray())) {
            $accountNumber = isset($data['originatorAccountNumber']) || isset($data['originator_account_number'])
                ? (string) ($data['originatorAccountNumber'] ?? $data['originator_account_number'])
                : null;
            $originatorAccount = new OriginatorAccount(
                new OriginatorIdentification(identification: $accountNumber)
            );
        }
